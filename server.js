...
io.on('connection', (socket) => {
  ...
  socket.on('updateStrimerName', strimer => {
    db.sequelize.query('UPDATE public."strimerName" SET name = :name', {replacements: {name: strimer}}).then(() => {
      db.sequelize.query('SELECT name FROM public."strimerName"').then(data => {
        io.emit('strimerName', data[0][0].name)
      })
    });
  });

  socket.on('getStrimerName', () => {
    db.sequelize.query('SELECT name FROM public."strimerName"').then(data => {
      io.emit('strimerName', data[0][0].name)
    })
  });
